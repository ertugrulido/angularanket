  ...
  AnketListele() {
    ...
    return this.aktifUye.pipe(
      concatMap((user) => {
        ...
        return collectionData(myQuery, { idField: 'anketId' }) as Observable<Gorev[]>;
      })
    );
  }
  AnketEkle(anket: Anketler) {
    var ref = collection(this.fs, "Anketler");
    return this.aktifUye.pipe(
      take(1),
      concatMap((user) =>
        addDoc(ref, {
          ...
          kayTarih:anket.kayTarih,
          duzTarih:anket.duzTarih,
          uid: user?.uid
        })
      ),
      map((ref) => ref.id)
